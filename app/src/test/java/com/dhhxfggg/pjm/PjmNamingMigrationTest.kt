...
class PjmNamingMigrationTest {
    ...
    private fun allFiles(): List<File> {
        fun walk(dir: File): List<File> =
            dir.listFiles()?.flatMap { if (it.isDirectory) walk(it) else listOf(it) } ?: emptyList()
        return walk(vaultRoot)
    }

    /** 在指定分类目录下建一个文件并写入内容，返回文件 */
    private fun createVaultFile(category: String, name: String, content: String = "pjm-data"): File {
    ...
    private suspend fun index(f: File, category: String) {
        dao.upsert(
            FileEntity(
    ...
    private fun runMigration(): Int = runBlocking {
    ...
    @Test
    fun legacyMillisAndNoVolumeSuffix_areNormalized_withIndexInSync() = runBlocking {
        // 旧式毫秒多卷
        ...
        val expected = listOf(
            "Export_${legacyTs}.pjm.1", "Export_${legacyTs}.pjm.2",
    ...
    // ---------- 2. 目标名已存在：安全跳过，不覆盖 ----------
    @Test
    fun targetAlreadyExists_isSkipped_withoutOverwrite() = runBlocking {
    ...
    @Test
    fun mixedRandomSample_noFileLost_noIndexLost() = runBlocking {
