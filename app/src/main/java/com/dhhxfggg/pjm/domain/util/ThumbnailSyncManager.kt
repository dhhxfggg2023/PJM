 ...
 */
@Singleton
class ThumbnailSyncManager @Inject constructor(
    ...
    fun scheduleSync(initialDelayMs: Long = 2000) {
        ...
        syncJob = MainApplication.applicationScope.launch {
            delay(initialDelayMs)
            // 有其他任务进行中 → 等待让位，不抢占进度条
            while (isActive && VaultManager.isOperationActive) delay(10_000)
            while (isActive) {
                if (VaultManager.isOperationActive) { delay(10_000); continue }
                val more = syncOnce()
                if (!more) break
                delay(1500)
            }
    ...
    private suspend fun syncOnce(): Boolean {
        ...
        val all = try { fileDao.getAllFiles().first() } catch (_: Exception) { return false }
